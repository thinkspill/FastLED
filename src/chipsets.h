...
template <uint8_t DATA_PIN, EOrder RGB_ORDER = RGB> class PixieController : public CPixelLEDController<RGB_ORDER>
{
...
class LPD8806Controller : public CPixelLEDController<RGB_ORDER>
{
    ...
    class LPD8806_ADJUST
    {
      public:
        ...
        __attribute__((always_inline)) inline static uint8_t adjust(FASTLED_REGISTER uint8_t data)
        {
...
class APA102Controller : public CPixelLEDController<RGB_ORDER>
{
  ...
  private:
    static inline void getGlobalBrightnessAndScalingFactors(PixelController<RGB_ORDER> &pixels, uint8_t *out_s0,
                                                            uint8_t *out_s1, uint8_t *out_s2, uint8_t *out_brightness)
    {
    ...
    inline void showPixelsGammaBitShift(PixelController<RGB_ORDER> &pixels)
    {
        ...
        while (pixels.has(1))
        {
            ...
            if ((r | g | b) != 0)
            {
                ...
                five_bit_hd_gamma_bitshift(r, g, b, r_scale, g_scale, b_scale, // Post-gamma scale.
                                           &r, &g, &b, &brightness);
            }
...
class APA102ControllerHD
    : public APA102Controller<DATA_PIN, CLOCK_PIN, RGB_ORDER, SPI_SPEED, kFiveBitGammaCorrectionMode_BitShift,
...
/// SK9822 controller class. It's exactly the same as the APA102Controller protocol but with a different END_FRAME and
/// default SPI_SPEED.
/// @tparam DATA_PIN the data pin for these LEDs
/// @tparam CLOCK_PIN the clock pin for these LEDs
/// @tparam RGB_ORDER the RGB ordering for these LEDs
...
template <uint8_t DATA_PIN, uint8_t CLOCK_PIN, EOrder RGB_ORDER = RGB, uint32_t SPI_SPEED = DATA_RATE_MHZ(24)>
class SK9822Controller : public APA102Controller<DATA_PIN, CLOCK_PIN, RGB_ORDER, SPI_SPEED,
                                                 kFiveBitGammaCorrectionMode_Null, 0x00000000, 0x00000000>
{
};
...
/// Defaults to ::DATA_RATE_MHZ(24)
template <uint8_t DATA_PIN, uint8_t CLOCK_PIN, EOrder RGB_ORDER = RGB, uint32_t SPI_SPEED = DATA_RATE_MHZ(24)>
class SK9822ControllerHD : public APA102Controller<DATA_PIN, CLOCK_PIN, RGB_ORDER, SPI_SPEED,
                                                   kFiveBitGammaCorrectionMode_BitShift, 0x00000000, 0x00000000>
{
};
...
// need the more tightly defined timeframes.
    ...
    (CLOCKLESS_FREQUENCY == 8000000 || CLOCKLESS_FREQUENCY == 16000000 || CLOCKLESS_FREQUENCY == 24000000) ||          \
